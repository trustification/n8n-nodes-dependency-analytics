...
export async function getMany({ ctx, itemIndex }: { ctx: IExecuteFunctions; itemIndex: number }) {
  ...
  const rules: SortRule[] = readSortRules(ctx, itemIndex, 'advisory', 'getMany');
  ...
  return [
    {
      json: {
        total: res?.total ?? finalItems.length,
        advisories: finalItems,
      },
    },
  ];
}
...
export async function analyze({ ctx, itemIndex }: { ctx: IExecuteFunctions; itemIndex: number }) {
  ...
  if (inputType === 'purls') {
    ...
    try {
      ...
      const rules: SortRule[] = readSortRules(ctx, itemIndex, 'advisory', 'analyze');
  ...
  if (mode === 'simplified') {
    const simplified = (Array.isArray(out) ? out : [out]).map((item: any) => {
      ...
      const simplifyPackages = (packages: any) =>
        Array.isArray(packages)
          ? packages.map((pkg: any) => ({
              id: pkg?.id ?? null,
              purl: Array.isArray(pkg?.purl)
                ? pkg.purl.map((p: any) => ({ uuid: p?.uuid ?? null, purl: p?.purl ?? null }))
                : null,
              licenses: Array.isArray(pkg?.licenses) ? pkg.licenses : null,
            }))
          : null;
